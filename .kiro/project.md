...
- **React Frontend** — 8-bit D&D-inspired SPA with pixel art styling. Nginx reverse proxy to backend. Image in ECR (`krombat/frontend`)
...
5. Victory → treasure loot modal with Secret content
...
- **To test changes**: push to Git → CI builds image → restart deployment with `kubectl rollout restart`
...
- `scripts/` — Utility scripts (watch-dungeon.sh)
