...
kctl patch dungeon "test-heal-$TS" --type=merge -p '{"spec":{"heroHP":50,"heroMana":8}}' &>/dev/null
sleep 2

log "Submitting abilities"
# Heal (mage: target=hero)
submit_attack "test-heal-$TS" "hero"
HP=$(kctl get dungeon "test-heal-$TS" -o jsonpath='{.spec.heroHP}')
MANA=$(kctl get dungeon "test-heal-$TS" -o jsonpath='{.spec.heroMana}')
...
wait_for "taunt" "[ \$(kctl get dungeon test-taunt-$TS -o jsonpath='{.spec.tauntActive}' 2>/dev/null) = '1' ]" 15 \
...
wait_for "backstab cooldown" "[ \"\$(kctl get dungeon test-backstab-$TS -o jsonpath='{.spec.backstabCooldown}' 2>/dev/null)\" = '3' ]" 15 \
  && pass "Rogue backstab: cooldown=3" || fail "Backstab cooldown"
BS_HP=$(kctl get dungeon "test-backstab-$TS" -o jsonpath='{.spec.monsterHP[0]}')
