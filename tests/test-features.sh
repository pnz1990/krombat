...
# --- Modifier test ---
log "Modifier test"
MOD=$(kctl get dungeon "test-mod-$TS" -o jsonpath='{.status.game.modifier}' 2>/dev/null)
...
kctl patch dungeon "test-fx-$TS" --subresource=status --type=merge -p '{"status":{"game":{"poisonTurns":2,"burnTurns":1,"stunTurns":1,"heroHP":200}}}' &>/dev/null
sleep 2

submit_attack "test-fx-$TS" "test-fx-$TS-monster-0"

wait_for "effects ticked" "[ \"\$(kctl get dungeon test-fx-$TS -o jsonpath='{.status.game.poisonTurns}' 2>/dev/null)\" != '2' ]" 20
FX_HP=$(kctl get dungeon "test-fx-$TS" -o jsonpath='{.status.game.heroHP}')
FX_POISON=$(kctl get dungeon "test-fx-$TS" -o jsonpath='{.status.game.poisonTurns}')
FX_STUN=$(kctl get dungeon "test-fx-$TS" -o jsonpath='{.status.game.stunTurns}')
...
log "Loot test"
# Game state now lives in status.game — patch via status subresource
kctl patch dungeon "test-loot-$TS" --subresource=status --type=merge -p '{"status":{"game":{"inventory":"[\"hppotion-rare\"]","heroHP":50}}}' &>/dev/null
sleep 2
submit_action "test-loot-$TS" "use-hppotion-rare"
INV=$(kctl get dungeon "test-loot-$TS" -o jsonpath='{.status.game.inventory}')
[ -z "$INV" ] || [ "$INV" = "[]" ] && pass "Item consumed: inventory empty" || fail "Inventory=$INV"
HP_AFTER=$(kctl get dungeon "test-loot-$TS" -o jsonpath='{.status.game.heroHP}')
...
for i in $(seq 1 10); do
  ...
  HP=$(kctl get dungeon "test-loot-$TS" -o jsonpath='{.status.game.monsterHP[0]}' 2>/dev/null || echo "30")
  [ "$HP" -le 0 ] && break
done
sleep 2
...
submit_attack "test-loot-$TS" "test-loot-$TS-monster-0"
sleep 2
DEAD_LOOT=$(kctl get dungeon "test-loot-$TS" -o jsonpath='{.status.game.lastLootDrop}' 2>/dev/null || echo "")
