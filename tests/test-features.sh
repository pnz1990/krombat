...
TS=$(date +%s)
trap 'teardown_backend_pf; kubectl delete dungeons test-mod-$TS test-fx-$TS test-loot-$TS --ignore-not-found --wait=false 2>/dev/null' EXIT
...
# --- Modifier test ---
log "Modifier test"
MOD=$(kubectl get dungeon "test-mod-$TS" -o jsonpath='{.spec.modifier}' 2>/dev/null)
...
kubectl patch dungeon "test-fx-$TS" --type=merge -p '{"spec":{"poisonTurns":2,"burnTurns":1,"stunTurns":1,"heroHP":200}}' &>/dev/null
sleep 2

submit_attack "test-fx-$TS" "test-fx-$TS-monster-0"

wait_for "effects ticked" "[ \"\$(kubectl get dungeon test-fx-$TS -o jsonpath='{.spec.poisonTurns}' 2>/dev/null)\" != '2' ]" 20
...
log "Loot test"
kubectl patch dungeon "test-loot-$TS" --type=merge -p '{"spec":{"inventory":"hppotion-rare","heroHP":50}}' &>/dev/null
sleep 2
...
HP_AFTER=$(kubectl get dungeon "test-loot-$TS" -o jsonpath='{.spec.heroHP}')
...
for i in $(seq 1 10); do
  ...
  HP=$(kubectl get dungeon "test-loot-$TS" -o jsonpath='{.spec.monsterHP[0]}' 2>/dev/null || echo "30")
  [ "$HP" -le 0 ] && break
done
sleep 2
...
submit_attack "test-loot-$TS" "test-loot-$TS-monster-0"
sleep 2
DEAD_LOOT=$(kubectl get dungeon "test-loot-$TS" -o jsonpath='{.spec.lastLootDrop}' 2>/dev/null || echo "")
